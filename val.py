...
def convert_to_coco_format(pose_entries, all_keypoints):
    ...
    for n in range(len(pose_entries)):
        ...
        for keypoint_id in pose_entries[n][:-2]:
            position_id += 1
            if position_id == 1:  # COCO中没有'neck'
                continue

            cx, cy, score, visibility = 0, 0, 0, 0  # 未找到关键点
            if keypoint_id != -1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--labels', type=str, required=True, help='关键点val标签的json路径')
    parser.add_argument('--output-name', type=str, default='detections.json',
                        help='检测到的关键点的输出json文件名')
    parser.add_argument('--images-folder', type=str, required=True, help='COCO val图像文件夹的路径')
    parser.add_argument('--checkpoint-path', type=str, required=True, help='检查点的路径')
    parser.add_argument('--multiscale', action='store_true', help='在多个尺度上平均推断结果')
    parser.add_argument('--visualize', action='store_true', help='显示关键点')
